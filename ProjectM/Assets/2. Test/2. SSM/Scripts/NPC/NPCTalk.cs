...
public class NPCTalk : MonoBehaviour
{
    ...
    private Dictionary<string, List<string>> substringList;       // 50글자씩 자른 npc woard
    private List<string> npcWoad;
    private string dictFirst;                                   //substringList 첫번째 키값 저장용
    private StringBuilder dictProgressSb;                       //substringList 진행중 키값 저장용
    ...
    private void SetComponent() // 시작 컴포넌트및 정보 세팅
    {
        ...
        substringList = new Dictionary<string, List<string>>();
        ...
        NpcWord();
        dictFirst = substringList.First().Key; //첫번쨰 키 저장
        dictProgressSb.Append(dialogueLV == 0 ? substringList.First().Key : substringList.Keys.ElementAt(dialogueLV)); // 진행중 키값 저장용
        npcTextMeshPro.text = substringList[dictFirst][0]; // npc 대사 설정  dialogueData[j].Id.ToString() +"_"+ dialogueData[j].Choice_Order_Number .id _ 글등장 순서
    }
    ...
    public void DictLastNumberAdd() // 텍스트 순서 체인지
    {
        ...
        if (substringList.ContainsKey(fruits[0] + "_" + (Convert.ToInt32(fruits[fruits.Length - 1]) + 1).ToString())) // npc mbti대화 textLv상승용
        {
        ...
        else if (substringList.ContainsKey(fruits[0] + "_" + "99")) // 마지막 대화 존재 여부 체크
        {
            if (dictProgressSb.Equals(fruits[0] + "_" + "99")) // 마지막 대화 체크
            {
                ...
                QuestInput();
                mbtis.Clear();
                if (substringList.ContainsKey((Convert.ToInt32(fruits[0]) + 1).ToString() + "_" + "1")) // 다음 퀘스트 대화 여부 체크
                {
                    dialogueLV++;
                    ...
                    npcAction.NPCStateChange(NpcState.COMPLET); // 대화창 닫기
                }
    ...
    {
        npcAction.NPCStateChange(NpcState.TALk);
        ...
        if (npcAction._npcState == NpcState.COMPLET) // 마지막 대화까지 수행
        {
            TalkOff();
            return 0;
        }
     
        if (i < substringList[dictProgressSb.ToString()].Count)
        {
            if (i == 0)
            {
                TalkOn();
            ...
            {
                npcTextMeshPro.text = substringList[dictProgressSb.ToString()][i];
            }


            if (i == substringList[dictProgressSb.ToString()].Count - 1)
            {
                SetChoiceText();
            }
        }
        ...
        if (npcID.Equals(newNPC.NPC_ID.ToString()))
        {
            for (int i = 0; i <= newNPC.ContinueDialogueCount; i++)
            {
                ...
                for (int j = 0; j < dialogueData.Count; j++)
                {
                    ...
                    substringList.Add(dialogueData[j].Choice_Bundle_Tag.ToString() + "_" + dialogueData[j].Choice_Order_Number.ToString(), npcWoad);
